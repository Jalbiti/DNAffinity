...
class Dataset:
    ...
    def featurize(self):
        if 'avg' in self.feats:
            tetra_avg = {line.split()[0] : np.array([float(x) for x in line.split()[1:]])  for line in open(f'/orozco/projects/proteinBinding/PBM_SELEX/input/new_avg.dat') if 'SHIFT' not in line}
...
class Model:
 ...
 #       for f in range(self.X.shape[1]):
	...
	# plt.savefig(f'output/{protein}/feature_importance.png')
